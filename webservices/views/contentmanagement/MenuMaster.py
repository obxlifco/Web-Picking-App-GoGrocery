...
class MenuAdd(generics.ListAPIView):
# """ Add New Page Menu """
...
class HeaderMenu(generics.ListAPIView):
		...
		for menus in menu:
			if menus.flag == 1:
				...
				if menus.category_id is  None:
					...
					menus_master.append(d1)
		sub_menu=EngageboostCmsMenus.objects.using(company_db).all().filter(isdeleted='n',flag=flag,company_website_id=website_id).filter(~Q(parent_id=0))	
		child_menus_master=[]
		for sub_menus in sub_menu:
			if sub_menus.category_id is  None:
				header_menu=EngageboostPages.objects.using(company_db).get(id=sub_menus.page_id)
				d1={'isdefault':header_menu.isdefault,'name':header_menu.page_name,'id':sub_menus.id,'parent_id':sub_menus.parent_id,'url':header_menu.url,'type': 'page'}
				child_menus_master.append(d1)
			else:
				header_menu=EngageboostCategoryMasters.objects.using(company_db).get(id=sub_menus.category_id)
...
class CategoriesListMenu(generics.ListAPIView):
# """ Categories Selected  """
	#///////////////////Fetch Single Row
