from webservices.models import EngageboostCompanyWebsites,EngageboostMenuMasters
from django.http import Http404
from django.db.models import Q
from webservices.serializers import MenuSerializer,CompanyWebsiteSerializer
...
class Menumaster(generics.ListAPIView):
# """ List all Groups with pagination,sorting and searching """
    def post(self, request, format=None):
        company_db = loginview.db_active_connection(request)
        print('company_db'+company_db)
        # /////////Create Query
        if request.data.get('search'):
            key=request.data.get('search')
            groups = EngageboostMenuMasters.objects.using(company_db).all().filter(module__icontains=key).order_by('-id')
            ...
            if(order_type=='+'):
                ...
                order='-'+order_by
            groups = EngageboostMenuMasters.objects.using(company_db).all().order_by(order)    
        else:
            groups = EngageboostMenuMasters.objects.using(company_db).all().order_by('-id')
        # /////////Create Pagination    
        page = self.paginate_queryset(groups)
        if page is not None:
            serializer = MenuSerializer(page, many=True)
            return self.get_paginated_response(serializer.data) 
        serializer = self.MenuSerializer(groups, many=True)
        return Response(serializer.data)
# All GlobalSettings rows fetch web services
class MenumasterSettings(APIView):
    def get(self, request, format=None):
        company_db = loginview.db_active_connection(request)
        settings = EngageboostMenuMasters.objects.using(company_db).all().filter(isdeleted='n',isblocked='n').order_by('-id')
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            serializer.save()
            obj = EngageboostMenuMasters.objects.using(company_db).latest('id')
            last_id = obj.id
...
class Glsettingsup(APIView):
    def get_object(self, pk,request):
        company_db = loginview.db_active_connection(request)
        try:
            return EngageboostMenuMasters.objects.using(company_db).get(pk=pk)
        except EngageboostMenuMasters.DoesNotExist:
            raise Http404
