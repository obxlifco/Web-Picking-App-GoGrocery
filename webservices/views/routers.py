...
class DB(generics.ListAPIView):
    def post(self, request, format=None):
        ...
        db_fetch = EngageboostCompanies.objects.using(company_db).get(db_host=str(host))
        external_db = {'ENGINE': 'django.db.backends.postgresql_psycopg2',
                            'NAME': 'pg_primemalltest','USER': 'primemall',
                            'PASSWORD': 'primemall',
                            'HOST': '192.168.0.65',
                            'PORT': ''}
...
def get_dname():
    ...
    db_fetch = EngageboostCompanies.objects.using(company_db).get(db_host=str(host))
    external_db = {'ENGINE': 'django.db.backends.postgresql_psycopg2',
                            'NAME': 'pg_primemall','USER': 'primemall',
                            'PASSWORD': 'primemall',
                            'HOST': '192.168.0.65',
                            'PORT': ''}
...
class DatabaseRouter(object):
    ...
    def db_for_read(self, model, **hints):
        ...
        # current_site = Site.objects.using(company_db).get_current()
        ...
        return 'web'
            ...
            # user = EngageboostCompanies.objects.using(company_db).get(db_host=host)
            # print(user.query)
