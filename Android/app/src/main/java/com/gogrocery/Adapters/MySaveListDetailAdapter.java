...
public class MySaveListDetailAdapter extends RecyclerView.Adapter<MySaveListDetailAdapter.MyViewHolder> {
    ...
    public MySaveListDetailAdapter(Context mContext,
                                   List<com.gogrocery.Models.WishlistModel.Data> mMyWishlistData, DatabaseHandler mDatabaseHandler, WishlistInterface mWishlistInterface) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        try {
            mMyViewHolderMain.tvItemName.setText(mDataInner.getProduct().getName());
            ...
            if (mDataInner.getProduct().getChannelPrice() == null) {
                ...
                holder.tvDiscountedPrice.setText("" + Constants.VARIABLES.CURRENT_CURRENCY + " " + Constants.twoDecimalRoundOff(Double.parseDouble(mDataInner.getProduct().getChannelPrice().getChannelPrice())));
            }
...
*/
        ...
        mMyViewHolderMain.rlPlus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int qty = Integer.parseInt(mMyViewHolderMain.tvQuantity.getText().toString());
                System.out.println("Rahul : MyCardAdapter : rlMinus : qty : " + qty);
                mMyViewHolderMain.tvQuantity.setText("" + (qty + 1));

                  mWishlistInterface.connectMain(mDataInner.getProduct().getId(), Integer.parseInt(mMyViewHolderMain.tvQuantity.getText().toString().trim()), position);

            }
        });

        mMyViewHolderMain.rlMinus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
