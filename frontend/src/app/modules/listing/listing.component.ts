...
@Component({
  ...
  styleUrls: ['./listing.component.css','./slider.scss']
})
...
export class ListingComponent implements OnInit {
  ...
  public listingType:string = 'Category';
  ...
  constructor(
    ...
    // this.metaTagService.addTag({name: 'keywords', content: 'Angular Project, Create Angular Project'});
    ...
    this._warehouseService.warehouseData$.subscribe(response => {
      ...
      if (isPlatformBrowser(this.platformId)) {
        this._route.params.subscribe(response => {
          this.p = 0;
          this.resetListing();
          let currentUrlPath = this._route.snapshot.url[0].path;
          name = this._route.snapshot.paramMap.get('slug');
          if (currentUrlPath == 'brand') {
            this.brand_slug = name;
            this.listingType = 'Brand';
          } else {
            this.category_slug = name;
          }  
  ...
  getListingdata(name) {
    ...
    // this._productService.getIdFromSlug(name,this.listingType).subscribe(response => {
    //   if (response && response['status'] == 1) {
		// this.filterTitle = response['name'];

		// let seoData: any = {};
		// seoData['metatitle']='Gogrocery: '+response.name;
		// seoData['title']='Gogrocery: '+response.name;
		// seoData['description'] = response['meta_description']!='' ?  response['meta_description'] : response['name'];
		// seoData['keywords'] = response.meta_keywords != '' ?  response.meta_keywords : response['name'];					
    ...
    if (this.brand_slug != '' && this.category_slug == '') {
      type = 'brand'
      // let filter_set = this.elasticData();
      // this.showElasticDataInUI(filter_set);
    ...
    // if(!this.brand_slug){
    this.global.getMenuListingpageSidebar(name, type).subscribe(result => {
      this.filter_data_non_variation = [];
      this.filter_data_variation = [];
      this.filter_price = [];
        ...
        if (item.status == 0) {
          ...
          if (item.child.length > 0 && item.field_name == "price") {
            ...
            if (item.is_variant == "true" && item.child.length > 0) {
              ...
              //       fil.child.map(chi => {
              //         if (enc["variations.field_value"].query.includes(',')) {
              //           let array = enc["variations.field_value"].query.split(',');
              //           for (let i = 0; i < array.length; i++) {
              //             if (chi.name == array[i]) {
              //               chi.is_checked = true;
              //               this.filter_selected.push(chi);
              //             }
              //           }

              //         } else {
              //           if (chi.name == enc["variations.field_value"].query) {
              //             chi.is_checked = true;
              //           }
              //         }
  ...
  elasticData(sortingvalue?: any) {
    ...
    /*if (this.category_id) {
      first_filter.push({ "match": { "category_id": this.category_id } });
     
    }*/
    if (this.category_slug) {
      //first_filter.push({ "match": { "category_slug": { "query": this.category_slug, "operator": "and" } }});
      first_filter.push({ "terms": {'category_slug.keyword': [this.category_slug] }});

    }
    
    if (this.brand_slug) {  
      first_filter.push({ "terms": {'brand_slug.keyword': [this.brand_slug] }});
      //first_filter.push({ "match": { "brand_slug": { "query": this.brand_slug, "operator": "and" } } });
    }
    /*this.filter_selected.forEach(response => {
       if(response['field_type'] == 'brand') {
         first_filter.push({ "match": { "brand": { "query":response['name'], "operator": "or" } } });
       }
    });*/
    ...
    /*first_filter.push({
      "term" : {
        "inventory.id" : this.warehouse_id
      }
    },{
      "range": {
            "inventory.stock": {
   ...
   // first_filter.push({ "match": { "inventory.id": this.warehouse_id } });
    //first_filter.push()
    ...
    //first_filter.push({ "match": { "channel_currency_product_price.warehouse_id": this.warehouse_id } });
    //first_filter.push({ "term": { "product_images.is_cover": 1 } });
    //first_filter.push({ "must_not": {"term": {"product_images.img": ""}}});


    /*first_filter.push({ "range": {
            "channel_currency_product_price.new_default_price": {
              "gt": 0,
    ...
    //         let array_filter = [];
    //         let field_name = this.filter_data_variation[i].field_name.toString();
    //         console.log(field_name);
    //         urlModifiedCall = true;
    //         first_filter.push({ "match": { "variations.field_name": field_name } });
    //         first_filter.push({ "match": { "variations.field_value": { "query": total_filter_data_variation } } });
    ...
    //must_not['must_not'] = {"term": {"product_images.img": ""}};
    ...
    /*let term_filter = {};
    term_filter["term"] = { "channel_currency_product_price.warehouse_id": this.warehouse_id };*/
    let final_value = {};
    //final_value = Object.assign(must_filter, (term_filter));
    ...
    //final_value['nested'] = nested_filter;
    


    //sfinal_value['term'] = { "channel_currency_product_price.warehouse_id": this.warehouse_id };

   /* console.log("******************** Final value ***********************");
    console.log(final_value);*/
    // console.log(should_filter)
    let filter_set = {
      "query":
        { 
          "bool": final_value,
  ...
  showElasticDataInUI(filter_set) {
  	this._globalService.showLoaderSpinner(true);
    //let prodata = this.localStorage.getItem('product_list') ? JSON.parse(this.localStorage.getItem('product_list')) : {};
    // console.log(filter_set);
    //this.global.getListingData(filter_set).subscribe(data => {
      ...
      if (data.hits.hits.length >= 1) {
        ...
        pro_list.forEach(item => {
            ...
            if (item._source.inventory) {
              item._source.inventory.forEach(inven => {
                ...
                if (inven.id == this.warehouse_id) {
                  ...
                  if (item._source.custom_fields) {
                    item._source.custom_fields.forEach(im => {
                      ...
                      custom_field.push({
                        ...
                        field_id:im.field_id,
                        value:im.value,
                        field_label_l:im.field_label_l
                      })
                    })
                  }
                ...
                /*if (prodata) {
                  prodata.map(pro => {
                    this.product_list.map(z => {
                      if (pro.id == z.id && pro.quantity != null && pro.quantity != z.quantity) {
                        z.quantity = pro.quantity;
                        z.IsCart = true;
                      }
                    })
                  })
          ...
          // let current_value=this.product_list[0]._source.category;
        })
      } else {
        // this._globalService.showToast('Product is not available in this category');
      }
  ...
  */
  ...
  onScroll() {
  	//alert("hiii");
    // console.log(this.config.currentPageCount+' d '+this.product_list.length)
    if(this.config.currentPageCount!=this.product_list.length){
