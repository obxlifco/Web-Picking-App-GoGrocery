...
export class PopupImgDirective {
    ...
    constructor(
                    public dialogRef: MatDialogRef<PopupImgDirective> ,
                    public dialog: MatDialog,
                    @Inject(MAT_DIALOG_DATA) public data: any,
                    public _globalService:GlobalService,
                ) {

                    if(!data['file_type']){ data['file_type']=1; }
    ...
    fileChoose(e: Event){
        ...
        for(var i=0;i < target.files.length; i++) {
            ...
            if(!this.check_file_ext(target.files[i].type) && this.data['file_type']==1){
                this._globalService.showToast('Only jpeg/png/gif files are allowed');
            }else if(!this.check_file_ext(target.files[i].type) && this.data['file_type']==2){
    ...
    uploadFromLibrary() {
        this.dialogRef.close();
        this.dialogImgLibRef = this.dialog.open(ImageUploadLibDialog,{data: this.data});
    ...
    onDropboxLoad(){
        ...
        if(this.data['file_type']==1){
            file_ext = ['.jpg', '.jpeg', '.png'];
        }else if(this.data['file_type']==2){
            file_ext = ['.pdf'];
        }   
    ...
    onDropboxSucc(files){
        ...
        console.log(files);
        if(!this.check_file_size(files[0].bytes)){
            this._globalService.showToast('Maximum 2MB of file size is allowed');
        }else{
            var url = '';
            url = files[0].link;
...
export class ImageUploadUrlDialog {
    ...
    constructor(
                public dialogRef: MatDialogRef<ImageUploadUrlDialog> ,
                public dialog: MatDialog,
                @Inject(MAT_DIALOG_DATA) public data: any
    ) {
        dialog.afterOpen.subscribe(() => {
...
export class ImageMetaDataDialog {
    ...
    constructor(
                public dialogRef: MatDialogRef<ImageMetaDataDialog> ,
                public dialog: MatDialog,
                @Inject(MAT_DIALOG_DATA) public data: any
                ) {
    ...
    insertMetaData(formVal){
        ...
        this.dialogRef.close(meta_content);
...
export class ImageUploadLibDialog implements OnInit {
    ...
    constructor(
                ...
                public dialogRef: MatDialogRef<ImageUploadLibDialog> ,
                public dialog: MatDialog,
                @Inject(MAT_DIALOG_DATA) public data: any
    ) {
        dialog.afterOpen.subscribe(() => {
        ...
        this.s3_link = GlobalVariable.S3_URL+this.data.type+'/200x200/';
    } 

    ngOnInit(){
        this.images=[];
        let that = this;
        this._global.libLoad(this.data.type).subscribe(
            data => {
                ...
                if(this.data.type=='category'){
                    if(this.data.imgFor=='banner_image') 
                    {
                       data.category_banner_image.forEach(function(item:any){
                          that.images.push({'name': item, 'selected': false });
                    ...
                    else if(this.data.imgFor=='image') 
                    {
                      ...
                      this.s3_link = GlobalVariable.S3_URL+this.data.type+'/100x100/';  
