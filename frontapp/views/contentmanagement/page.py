...
class LoadPage(APIView):
    ...
    def get(self, request,pk, format=None):
        company_db = loginview.db_active_connection(request)
        if int(pk) == int(1):
            allpage = EngageboostPages.objects.using(company_db).filter(isdeleted='n',isblocked='n',company_website_id=pk).filter(~Q(id__in=[1,2,11,15]))
            # print('+++++++++',allpage.count())
        else:
            allpage = EngageboostPages.objects.using(company_db).filter(isdeleted='n',isblocked='n',company_website_id=pk)
...
def amazons3_temp_image(directory_name,file_name):
   #print(settings)
   conn = tinys3.Connection('AKIAI4SWZ5AI6HWTP5WQ','dW9eWerPrdoHf50VEfElJDyp0Jg3Z0nb2iYdAPD0',tls=True)
   f400 = open(settings.MEDIA_ROOT+'/temp_image/'+directory_name+'/'+file_name,'rb')
   conn.upload('Lifco/lifco/temp_image/'+directory_name+'/'+file_name,f400,'boostmysale')
   return 0
...
def Pageslurtoidload(request):
    ...
    try:
        ...
        pages=EngageboostCmsPageSettings.objects.using(company_db).filter(lang=lang,company_website_id=company_website_id,temp_id=template_id,page_id=page_slug.id).order_by('id')
