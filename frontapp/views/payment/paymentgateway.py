...
def CcAvenueCreateTransaction(order_id, si_ref_no=None):
    order_id = order_id
    rs_order    = EngageboostOrdermaster.objects.filter(id=order_id).first()
    order_data  = ViewOrderSerializer(rs_order)
    order_data  = order_data.data
    ...
    if si_ref_no != None:
        ...
        si_customer = EngageboostUserSIList.objects.filter(si_ref_no=si_ref_no)
        if si_customer.count() > 0:
            customer_id = si_customer.first().customer_id

            si_customer.update(is_default='y')
            EngageboostUserSIList.objects.filter(customer_id=customer_id).exclude(si_ref_no=si_ref_no).update(
                is_default='n')
...
def CcAvenueCreateTransactionByCustomOrderId(order_id):
    order_id = order_id
    ...
    EngageboostOrdermaster.objects.filter(custom_order_id=order_id).update(**update_order_master)
    common.save_data_to_elastic(rs_order.id, "EngageboostOrdermaster")
    rs_trans = EngageboostCcavenueUpgradedReturnDetails.objects.filter(order_id=rs_order.id).first()
    insert_id = 0
    if rs_trans:
        rs_record = EngageboostCcavenueUpgradedReturnDetails.objects.filter(order_id=rs_order.id).update(**transaction_arr)
        insert_id = rs_record
    else:
        transaction_arr.update({"order_status":"pending"})
        rs_record = EngageboostCcavenueUpgradedReturnDetails.objects.filter(order_id=rs_order.id).create(**transaction_arr)
        insert_id = rs_record.id
    data = {
        "customer_id":order_data['customer']['id'],
        "custom_order_id":order_data['custom_order_id'],
...
class CcAvenueOrderTransaction(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        si_ref_no = requestdata.get('si_ref_no', None)
        ...
        if order_id is not None and int(order_id)>0:
            res_ccavn_trans = CcAvenueCreateTransaction(order_id, si_ref_no)
            if res_ccavn_trans:
...
def UpdateCcAvenuePaymentTransaction_new(request):
    response_data = request
    ...
    try:
        ...
        # order_id=response_data['order_id']
        rs_order = EngageboostOrdermaster.objects.filter(custom_order_id=response_data['order_id']).first()
        user_id = rs_order.customer.auth_user_id
        order_id = rs_order.id
        ...
        if response_data["order_status"].lower() == "success":   # Payment Success
            ...
            update_arr = {
                ...
                "currency":response_data['currency'],
                "modified":now_utc,
                "si_ref_no":response_data['si_ref_no']
            }
            update_ordermaster = {
                # "order_id":order_id,
                "custom_order_id":rs_order.custom_order_id,
            ...
            common.email_send_by_AutoResponder(order_id, 3)
            ...
            common.email_send_by_AutoResponder(order_id, 28)

        UpdateOrdermasterElastic(update_ordermaster)
        rs_record = EngageboostCcavenueUpgradedReturnDetails.objects.filter(order_id=order_id).update(**update_arr)
        rs_order_record = EngageboostOrdermaster.objects.filter(id=order_id).update(**update_ordermaster)
    except Exception as error:
        trace_back = sys.exc_info()[2]
        line = trace_back.tb_lineno
        data = {"status":0,"api_status":traceback.format_exc(),"error_line":line,"error_message":str(error),"message": str(error),"req_data":response_data}
...
def UpdateCcAvenuePaymentTransaction_si(request):
    response_data = request
    # user_id = request.user.id
    pay_status  = ""
    currency_code = "AED"
    ...
    try:
        ...
        if response_data["order_status"].lower() == "success":   # Payment Success
            ...
            si_create = EngageboostUserSIList.objects.create(si_ref_no=response_data['si_ref_no'],
                                                             customer_id=rs_order.customer_id)
            if si_create:
                EngageboostUserSIList.objects.exclude(si_ref_no=response_data['si_ref_no']).filter(
                    customer_id=rs_order.customer_id).update(is_default='n')

            common.email_send_by_AutoResponder(order_id, 3)
            common.sms_send_by_AutoResponder(order_id, '', 3, '')
            ...
            # common.email_send_by_AutoResponder(order_id, 28)
            common.email_send_by_AutoResponder(order_id, 25)
            common.sms_send_by_AutoResponder(order_id, '', 25, '')
...
class TestCC(APIView):
    permission_classes = []
    def post(self, request, *args, **kwargs):
        res_data  = update_ccavenue_order_status_cron_test()
        return Response(res_data)

def update_ccavenue_order_status_cron_test_v1():
    ...
    try:
        ...
        if int(str_json['Order_Lookup_Result']['total_records']) >0:
            ...
            # query_lst.append(rs_order.query)
            print("rs_order.query====", rs_order.query)
            ...
            if len(ccavenue_order_data)>0 and len(rs_order_lst)>0:
                for order_lookup_data in ccavenue_order_data:
                    ...
                    if int(order_id) in rs_order_lst:
                        order_status.append(order_lookup_data["order_status"])
                        if order_lookup_data["order_status"].lower()!="awaited":                    
                            ...
                            if order_lookup_data["order_status"].lower()=="shipped" or order_lookup_data["order_status"].lower()=="successful" or order_lookup_data["order_status"].lower()=="success":
                                ...
                                rs_order = EngageboostOrdermaster.objects.filter(id=order_id).first()
                                rs_chk_chipment = EngageboostShipmentOrders.objects.filter(order_id=order_id,shipment_status='Invoicing',shipment=rs_order.shipment_id).first()
                                if rs_chk_chipment:
                                    effected_order_ids.append(order_id)
                            ret_order_ids.append(update_order_arr)
                            # EngageboostOrdermaster.objects.filter(id=order_id).update(**update_order_arr)

        data = {'ccavenue_order_data':ccavenue_order_data, 'ret_order_ids':ret_order_ids,'rs_order_lst':rs_order_lst, "effected_order_ids":effected_order_ids}
    except Exception as error:
        trace_back = sys.exc_info()[2]
        line = trace_back.tb_lineno
...
def update_ccavenue_order_status_cron():
    ...
    try:
        ...
        if int(str_json['Order_Lookup_Result']['total_records']) >0:
            ccavenue_order_data = str_json['Order_Lookup_Result']['order_Status_List']['order']
            rs_order = EngageboostOrdermaster.objects.filter(created__date__gte = current_date, order_status =21, buy_status=1, payment_method_id=51, payment_type_id=2).values_list("id", flat=True)
            rs_order_lst = list(rs_order)
            if len(ccavenue_order_data)>0 and len(rs_order_lst)>0:
                for order_lookup_data in ccavenue_order_data:
                    order_id = str(order_lookup_data["order_no"])[-4:]
                        ...
                        if order_lookup_data["order_status"].lower()!="awaited":                    
                            ...
                            if order_lookup_data["order_status"].lower()=="shipped" or order_lookup_data["order_status"].lower()=="successful" or order_lookup_data["order_status"].lower()=="success":
                                ...
                                if rs_chk_chipment:
                                    ...
                                    EngageboostOrdermaster.objects.filter(id=order_id).update(**update_order_arr)
                                    data = effected_order_ids
    except Exception as error:
        trace_back = sys.exc_info()[2]
        line = trace_back.tb_lineno
        data={"status":0,"api_status":traceback.format_exc(),"error_line":line,"error_message":str(error)}
    print("CC Avenue===================",data)

def update_ccavenue_order_status_cron_test(json_data = None):
    # access_code = 'AVZO03HA32BC46OZCB'                   # Live
    # workingKey = '1D5E76DE84FD6BD8B7708CF1A3A411C0'     # Live
    # url = "https://login.ccavenue.ae/apis/servlet/DoWebTrans"
    now_utc = datetime.now(timezone.utc).astimezone()
    ...
    try:
        ...
        json_data = json_data
        str_json = {}
        str_json.update({"Order_Lookup_Result":{"total_records":1}})
        
        str_json.update({"Order_Lookup_Result":{"order_Status_List":{"order":json_data}}})
        ccavenue_order_data = []
        data                = {}
        rs_order_lst        = []
        effected_order_ids  = []
        order_status        = []
        query_lst           = []
        str_json['Order_Lookup_Result']['total_records'] = 1
        # if int(str_json['Order_Lookup_Result']['total_records']) >0:
        if len(str_json['Order_Lookup_Result']['order_Status_List']['order']) >0:
            ccavenue_order_data = str_json['Order_Lookup_Result']['order_Status_List']['order']
            check_order_id_lst = []
            if len(ccavenue_order_data)>0:
                for ccave_orderlist in ccavenue_order_data:
                    if len(str(ccave_orderlist['order_no']))>3 and len(str(ccave_orderlist['order_no']))<8:
                        cust_order_id = str(ccave_orderlist['order_no'])
                        cust_order_id = "5"+cust_order_id.zfill(7)
                        check_order_id_lst.append(str(cust_order_id))
                    else:
                        check_order_id_lst.append(str(ccave_orderlist['order_no']))

            # rs_order = EngageboostOrdermaster.objects.filter(created__date__gte = current_date, order_status =100, buy_status=1, payment_method_id=51, payment_type_id=2).values_list("id", flat=True)
            rs_order = EngageboostOrdermaster.objects.filter( order_status =100, buy_status=1,payment_method_id=51, payment_type_id=2, custom_order_id__in = check_order_id_lst).values_list('custom_order_id', flat=True)
            rs_order_lst = list(rs_order)
            if len(ccavenue_order_data)>0 and len(rs_order_lst)>0:
                for order_lookup_data in ccavenue_order_data:
                    custom_order_id = str(order_lookup_data["order_no"])                   
                    if str(custom_order_id) in rs_order_lst:
                        print('custom_order_id==========', custom_order_id)
                        order_status.append(order_lookup_data["order_status"])
                        if order_lookup_data["order_status"].lower()!="awaited":                    
                            update_order_arr = {
                                "pay_txntranid":order_lookup_data["reference_no"],
                            ...
                            if order_lookup_data["order_status"].lower()=="shipped" or order_lookup_data["order_status"].lower()=="successful" or order_lookup_data["order_status"].lower()=="success":
                                ...
                                rs_order = EngageboostOrdermaster.objects.filter(custom_order_id=custom_order_id).first()
                                order_id = rs_order.id
                                rs_chk_chipment = EngageboostShipmentOrders.objects.filter(order_id=rs_order.id,shipment_status='Invoicing',shipment=rs_order.shipment_id).first()
                                if rs_chk_chipment:
                                    EngageboostShipmentOrders.objects.filter(order_id=order_id,shipment_status='Invoicing',shipment=rs_order.shipment_id).update(shipment_status='Packed')                                   
                                    EngageboostShipments.objects.filter(id=rs_order.shipment_id,shipment_status='Invoicing').update(shipment_status='Packed')
                                    # EngageboostTrentPicklists.objects.filter(id=rs_order.trent_picklist_id,picklist_status='Invoicing').update(picklist_status='Packed')
        ...
        data = {'ccavenue_order_data':ccavenue_order_data, 'rs_order_lst':rs_order_lst, "effected_order_ids":effected_order_ids}
    except Exception as error:
        trace_back = sys.exc_info()[2]
        line = trace_back.tb_lineno
        data={"status":0,"api_status":traceback.format_exc(),"error_line":line,"error_message":str(error)}
...
def GetPgResponseByDate(date=None):
    access_code = 'AVZO03HA32BC46OZCB'                   # Live
    workingKey = '1D5E76DE84FD6BD8B7708CF1A3A411C0'     # Live
    # merchant_id = "46319"                               # Live
    ...
    try:
        ...
        ret_data = {"obj_resp":obj_resp}
    except Exception as error:
        trace_back = sys.exc_info()[2]
        line = trace_back.tb_lineno
        ret_data={"status":0,"api_status":traceback.format_exc(),"error_line":line,"error_message":str(error)}

    return ret_data

def CheckTest():
    url = "https://www.gogrocery.ae/api/front/v1/test-cc/"
    r = requests.post(url = url) 
    url_response = r.json() 
    json_data =  url_response['ccavenue_order_data']         
    update_ccavenue_order_status_cron_test(json_data)
