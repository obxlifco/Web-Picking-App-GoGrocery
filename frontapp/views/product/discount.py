...
class ProductDiscountCalculation(APIView):
    def post(self, request, format=None):
        ...
        for index in range(len(product_ids)):
            ...
            for getproductforcartnet in getproductforcart_net:
                ...
                try:
                    if getproductforcartnet["new_default_price"]:
                        getproductforcartnet["new_default_price"] = getproductforcartnet["new_default_price"]
                    else:
            ...
            tax_price_arr = get_price_including_tax(website_id, company_id, product_detail_net, country_id, state_id,
                                                    post_code, 'back')
            ...
            if str(company_state) != str(state_id):
                product_tax = {"tax_price_unit": float(tax_price_arr["tax_igst"]),
                               "tax_price": float(tax_price_arr["tax_igst"]) * float(product_detail_net["qty"]),
                               "tax_percentage": tax_price_arr["igst"], "tax_name": tax_price_arr["tax_name"],
                               "cgst": tax_price_arr["cgst"], "sgst": tax_price_arr["sgst"],
                               "igst": tax_price_arr["igst"], "cess": tax_price_arr["cess"], "is_igst": "yes",
                               "tax_type": tax_price_arr["tax_type"]}
            else:
                product_tax = {"tax_price_unit": float(tax_price_arr["tax_igst"]),
                               "tax_price": float(tax_price_arr["tax_igst"]) * float(product_detail_net["qty"]),
                               "tax_percentage": tax_price_arr["igst"], "tax_name": tax_price_arr["tax_name"],
                               "cgst": tax_price_arr["cgst"], "sgst": tax_price_arr["sgst"],
                               "igst": tax_price_arr["igst"], "cess": tax_price_arr["cess"], "is_igst": "no",
                               "tax_type": tax_price_arr["tax_type"]}

            product_detail_net = dict(product_detail_net, **product_tax)

            if product_detail_net["tax_type"] == "including":
                product_detail_net["new_default_price_unit"] = float(
                    product_detail_net["new_default_price_unit"]) - float(product_detail_net["tax_price_unit"])
                product_detail_net["new_default_price"] = float(product_detail_net["new_default_price"]) - float(
                    product_detail_net["tax_price"])
            # ********Tax Calculation (END)********#
        ...
        if cartdetails:
            for products in cartdetails:
                if products["tax_type"] == "including":
                    order_total = float(order_total) + float(products["new_default_price"]) + float(
                        products["tax_price"])
                else:
        ...
        shipping_amount_arr = {"shipping_amount": 0, "mthod_type": 0, "mthod_name": "orderwise",
                               "handling_fees_type": 0, "handling_price": 0}
        if cartdetails:
            ...
            if shipping_flat:
                if float(shipping_flat["flat_price"]) > float(0) and float(order_total) < float(
                        shipping_free["minimum_order_amount"]):
                    shipping_amount_arr = {"shipping_amount": float(shipping_flat["flat_price"]),
                                           "mthod_type": shipping_flat["mthod_type"],
                                           "mthod_name": shipping_flat["mthod_name"],
                                           "handling_fees_type": shipping_flat["handling_fees_type"],
                                           "handling_price": shipping_flat["handling_price"]}

            if shipping_table:
                if float(shipping_table["flat_price"]) > float(0) and float(order_total) < float(
                        shipping_free["minimum_order_amount"]):
                    shipping_amount_arr = {"shipping_amount": float(shipping_table["flat_price"]),
                                           "mthod_type": shipping_table["mthod_type"],
                                           "mthod_name": shipping_table["mthod_name"],
                                           "handling_fees_type": shipping_table["handling_fees_type"],
                                           "handling_price": shipping_table["handling_price"]}
            ...
            if discount_array_coupon and discount_array_coupon[0]["name"] != "invalid":
                if discount_array_coupon[0]["disc_type"] != 3:
                    if discount_array_coupon[0]["coupon_type"] == 2:
                        cartdetails = genrate_new_prodcut_with_discount_coupon(user_id, cartdetails,
                                                                               discount_array_coupon, order_total)
                    else:
                    ...
                    if discount_array_coupon[0]["coupon_type"] == 2:
                        coupon_details = genrate_new_prodcut_with_discount_coupon_order_total(user_id, cartdetails,
                                                                                              discount_array_coupon,
                                                                                              order_total)
                    else:
                        if discount_array_coupon[0]["used_coupon"] == 0:
                            coupon_details = genrate_new_prodcut_with_discount_coupon_order_total(user_id, cartdetails,
            ...
            if coupon_code is not None:
                if discount_array_coupon and discount_array_coupon[0]["name"] != "invalid":
                    ...
                    applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                           ...
                                           "amount": discount_array_coupon[0]["amount"]}
                    ...
                    applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                           "coupon_code": discount_array_coupon[0]["coupon_code"],
                                           "message": discount_array_coupon[0]["message"]}
                    applied_coupon.append(applied_coupon_dict)
            else:
                grandtotal = float(grandtotal)
                subtotal = float(grandtotal)
        ...
        nettotal = float(subtotal) - float(total_tax) - float(shipping_amount) - float(handling_charge) - float(
            applied_cod_charge)
        balance_due = float(subtotal) - float(paid_amount)
        orderamountdetails_dict = {"company_id": company_id, "website_id": website_id, "webshop_id": webshop_id,
                                   "payment_method_id": payment_method_id, "tax_amount": total_tax,
                                   "shipping_charge": shipping_amount, "handling_charge": handling_charge,
                                   "cod_charge": applied_cod_charge, "gross_discount": gross_discount_amount,
                                   "min_amount": cod_min_amount, "max_amount": cod_max_amount,
                                   "grand_total": grandtotal, "sub_total": subtotal, "net_total": nettotal,
                                   "cart_discount": cart_discount, "paid_amount": paid_amount,
                                   "balance_due": balance_due}
        orderamountdetails.append(orderamountdetails_dict)
        data = {"cartdetails": cartdetails, "orderamountdetails": orderamountdetails, "applied_coupon": applied_coupon,
                "shipping_flat": shipping_flat, "shipping_table": shipping_table}
    ...
    if user_group_id:
        if warehouse_id is not None:
            DiscountMasterCond = EngageboostDiscountMasters.objects.order_by("discount_priority").filter(
                website_id=website_id, disc_start_date__lte=now_utc, disc_end_date__gte=now_utc,
                customer_group__iregex=r"\y{0}\y".format(user_group_id), isdeleted='n', isblocked='n',
                discount_master_type=0, warehouse_id__iregex=r"\y{0}\y".format(warehouse_id)).all()
        else:
            DiscountMasterCond = EngageboostDiscountMasters.objects.order_by("discount_priority").filter(
                website_id=website_id, disc_start_date__lte=now_utc, disc_end_date__gte=now_utc,
                customer_group__iregex=r"\y{0}\y".format(user_group_id), isdeleted='n', isblocked='n',
                discount_master_type=0).all()
    else:
        if warehouse_id is not None:
            DiscountMasterCond = EngageboostDiscountMasters.objects.order_by("discount_priority").filter(
                website_id=website_id, disc_start_date__lte=now_utc, disc_end_date__gte=now_utc, isdeleted='n',
                isblocked='n', discount_master_type=0, warehouse_id__iregex=r"\y{0}\y".format(warehouse_id)).all()
        else:
            DiscountMasterCond = EngageboostDiscountMasters.objects.order_by("discount_priority").filter(
                website_id=website_id, disc_start_date__lte=now_utc, disc_end_date__gte=now_utc, isdeleted='n',
                isblocked='n', discount_master_type=0).all()
    if discountIds:
...
def getproductforcart(product_id, warehouse_id=None):
    ...
    qs_price_data = EngageboostChannelCurrencyProductPrice.objects.filter(product_id=product_id,
                                                                          warehouse_id=warehouse_id,
                                                                          product_price_type_id__price_type_id=1).first()
...
def genrate_new_prodcut_with_discount(user_id=None, product_array=None, discount_array_new=None, category_id=None,
                                      sub_category_id=None, cart_subtotal=None):
    ...
    if discount_array_new:
        for discount_array in discount_array_new:
            ...
            return_price = check_in_prod_disc(user_id, product_array, discount_array, category_id, sub_category_id,
                                              cart_subtotal)
            price_array = return_price.split('^')
            if price_array[2] != "false":
                if price_array[1] != "product" and float(price_array[1]) != 0.0 and float(
                        product_array["channel_price"]) != float(price_array[0]):
                    product_array["new_default_price_unit"] = float(product_array["channel_price"]) - float(
                        price_array[1])
                    product_array["new_default_price"] = (float(product_array["channel_price"]) - float(
                        price_array[1])) * float(
                        product_array["qty"])  # float(price_array[0])*float(product_array["qty"])
                    ...
                    product_array["new_default_price_unit"] = float(product_array["channel_price"]) if product_array[
                        "channel_price"] else 0.0
                    if product_array["default_price"] and product_array["qty"]:
                        product_array["new_default_price"] = float(product_array["channel_price"]) * int(
                            product_array["qty"])
                    else:
        ...
        # for product in product_array:
        product_array["new_default_price_unit"] = float(product_array["default_price"]) if product_array[
                                                                                               "default_price"] is not None else 0
...
def check_in_prod_disc(user_id=None, ind_product=None, discount_array=None, category_id=None, sub_category_id=None,
                       cart_subtotal=None):
    ...
    if previos_flag == "true":
        if discount_array["disc_type"] == 1:
            discount_array["discountPrice"] = float(ind_product["default_price"]) * float(
                discount_array["amount"]) / float(100)
        else:
...
def get_price_including_tax(website_id='1', company_id=None, product_arr=None, country_id=None, state_id=None,
                            post_code=None, order_from='front'):
    state_id = state_id
    tax_price_arr = []
    tax_price = "0.00"
    excise_duty = "0.00"
    ...
    if product_arr["hsn_id"]:
        ...
        if hsnDetails:
            ...
            if tax_type == "excluding":
                ...
                data = {"tax_cgst": float(tax_price) / float(2), "tax_sgst": float(tax_price) / float(2),
                        "tax_igst": float(tax_price), "tax_name": hsnDetails.hsn_code,
                        "cgst": float(tax_rate) / float(2), "sgst": float(tax_rate) / float(2), "igst": float(tax_rate),
                        "cess": float(hsnDetails.cess), "tax_type": "excluding"}
            else:
                tax_price = (float(product_arr["new_default_price_unit"]) * float(tax_rate)) / (
                            float(100) + float(tax_rate))
                data = {"tax_cgst": float(tax_price) / float(2), "tax_sgst": float(tax_price) / float(2),
                        "tax_igst": float(tax_price), "tax_name": hsnDetails.hsn_code,
                        "cgst": float(tax_rate) / float(2), "sgst": float(tax_rate) / float(2), "igst": float(tax_rate),
                        "cess": float(hsnDetails.cess), "tax_type": "including"}
        else:
            data = {"tax_cgst": float(0), "tax_sgst": float(0), "tax_igst": float(0), "tax_name": "", "cgst": float(0),
                    "sgst": float(0), "igst": float(0), "cess": float(0), "tax_type": ""}
    else:
        data = {"tax_cgst": float(0), "tax_sgst": float(0), "tax_igst": float(0), "tax_name": "", "cgst": float(0),
                "sgst": float(0), "igst": float(0), "cess": float(0), "tax_type": ""}
...
def get_price_including_tax_vat(website_id='1', company_id=None, product_arr=None, country_id=None, state_id=None,
                                post_code=None, order_from='front'):
    state_id = state_id
    tax_price_arr = []
    tax_price = "0.00"
    ...
    is_tax = False
    tax_settings = EngageboostTaxSettings.objects.filter(website_id=1).values('catalog_prices').first()
    if product_arr['taxclass']:
        taxclassId = str(product_arr['taxclass']['id'])
        tax_rule_obj = EngageboostTaxRuleTables.objects.filter(isblocked='n', isdeleted='n')
        tax_rule_obj = tax_rule_obj.filter(product_tax_class_id__iregex=r"\y{0}\y".format(taxclassId))
        if post_code is not None and post_code != "":
            tax_rule_obj = tax_rule_obj.filter(tax_rate_id__zipcode__isnull=False,
                                               tax_rate_id__zipcode__iregex=r"\y{0}\y".format(post_code))
            if tax_rule_obj.count() > 0:
                is_tax = True

        elif state_id is not None and state_id != "":
            tax_rule_obj = tax_rule_obj.filter(tax_rate_id__state_id__isnull=False,
                                               tax_rate_id__state_id__iregex=r"\y{0}\y".format(state_id))
            if tax_rule_obj.count() > 0:
                is_tax = True

        else:
            if country_id is not None and country_id != "":
                tax_rule_obj = tax_rule_obj.filter(tax_rate_id__country_id=country_id)

        tax_rule_obj = tax_rule_obj.order_by('priority')

        tax_rule_obj = tax_rule_obj.values('tax_rate_id').first()
        if tax_rule_obj:
            tax_rate_obj = EngageboostTaxRates.objects.filter(id=tax_rule_obj['tax_rate_id'], isblocked='n',
                                                              isdeleted='n')
            if tax_rate_obj.count() > 0:
                tax_rates = tax_rate_obj.values('percentage', 'name').first()
                tax_rate = tax_rates['percentage']
                tax_name = tax_rates['name']
                tax_price = float(product_arr["new_default_price_unit"]) * float(tax_rate) / float(100)
                data = {"tax_name": tax_name, "tax_rate": tax_rate, "tax_type": tax_settings['catalog_prices']}
            else:
                data = {"tax_name": tax_name, "tax_rate": tax_rate, "tax_type": ""}
        else:
            data = {"tax_name": tax_name, "tax_rate": tax_rate, "tax_type": ""}
    else:
        data = {"tax_name": tax_name, "tax_rate": tax_rate, "tax_type": ""}
...
def rate_flat(website_id='1', company_id=None, country_id=None, state_id=None, post_code=None, order_from='front',
              warehouse_id=None, str_from=None):
    # ******** FOR FLAT RATE CALCULATION ********#
    zone_id = ''
    isShippingChargeFound = False
    if warehouse_id is not None:
        zonemaster_exists = EngageboostZoneMasters.objects.filter(warehouse_id=warehouse_id, location_type='Z',
                                                                  isblocked='n', isdeleted='n').exists()
        if zonemaster_exists:
            zonemaster_qs = EngageboostZoneMasters.objects.filter(warehouse_id=warehouse_id, location_type='Z',
                                                                  isblocked='n', isdeleted='n').latest('id')
    ...
    data = {"shipping_type": "Flat Shipping", "mthod_type": 0, "mthod_name": "orderwise", "flat_price": float(0),
            "handling_fees_type": 0, "handling_price": float(0)}

    if state_id and zone_id and zone_id != '':
        conditions = EngageboostShippingMastersSettings.objects.filter(website_id=website_id,
                                                                       country_ids__iregex=r"\y{0}\y".format(
                                                                           country_id),
                                                                       zone_id__iregex=r"\y{0}\y".format(zone_id),
                                                                       shipping_method_id='4', isblocked='n',
                                                                       isdeleted='n').all()
        ...
        conditions = EngageboostShippingMastersSettings.objects.filter(website_id=website_id,
                                                                       country_ids__iregex=r"\y{0}\y".format(
                                                                           country_id),
                                                                       state_id__iregex=r"\y{0}\y".format(state_id),
                                                                       shipping_method_id='4', isblocked='n',
                                                                       isdeleted='n').all()
        isShippingChargeFound = True
    ...
    if isShippingChargeFound:
        ...
        if conditions:
                ...
                if settingsinfo["mthod_type"]:
                    if settingsinfo["mthod_type"] == "1":  # ****ITEMWISE
                        if float(settingsinfo["flat_price"]) > float(0):
                            data = {"shipping_type": "Flat Shipping", "shipping_setting_id": settingsinfo["id"],
                                    "mthod_type": 1, "mthod_name": "itemwise",
                                    "flat_price": float(settingsinfo["flat_price"])}
                        else:
                            data = {"shipping_type": "Flat Shipping", "mthod_type": 1, "mthod_name": "itemwise",
                                    "flat_price": float(0)}
                    else:
                        if float(settingsinfo["flat_price"]) > float(0):
                            data = {"shipping_type": "Flat Shipping", "shipping_setting_id": settingsinfo["id"],
                                    "mthod_type": 0, "mthod_name": "orderwise",
                                    "flat_price": float(settingsinfo["flat_price"])}
                        else:
                            data = {"shipping_type": "Flat Shipping", "mthod_type": 0, "mthod_name": "orderwise",
                                    "flat_price": float(0)}
                else:
                    data = {"shipping_type": "Flat Shipping", "mthod_type": 0, "mthod_name": "orderwise",
                            "flat_price": float(0)}
                ...
                if settingsinfo["handling_fees_type"] is not None:
                    if settingsinfo["handling_fees_type"] == 0:  # ****FIXED PRICE
                        if float(settingsinfo["handling_price"]) > float(0):
                            d1 = {"shipping_type": "Flat Shipping", "handling_fees_type": 0,
                                  "handling_price": float(settingsinfo["handling_price"])}
                        else:
                            d1 = {"shipping_type": "Flat Shipping", "handling_fees_type": 0, "handling_price": float(0)}
                    else:
                        if float(settingsinfo["handling_price"]) > float(0):
                            d1 = {"shipping_type": "Flat Shipping", "handling_fees_type": 1,
                                  "handling_price": float(settingsinfo["handling_price"])}
                        else:
            ...
            if str_from is None:
                data = {"shipping_type": "Flat Shipping", "mthod_type": 0, "mthod_name": "orderwise",
                        "flat_price": float(shipping_charge), "handling_fees_type": 0, "handling_price": float(0)}
            else:
                data = {"shipping_type": "Flat Shipping", "mthod_type": 0, "mthod_name": "orderwise", "flat_price": 0,
                        "handling_fees_type": 0, "handling_price": float(0)}
        # ----Binayak End----#
    else:
        data = {"shipping_type": "Flat Shipping", "mthod_type": 0, "mthod_name": "orderwise",
...
def rate_table(website_id='1', company_id=None, cartdetails=None, country_id=None, state_id=None, post_code=None,
               order_from='front', str_from=None):
    data = {"shipping_type": "Table Rate Shipping", "mthod_type": 0, "mthod_name": "orderwise", "flat_price": float(0),
            "handling_fees_type": 0, "handling_price": float(0)}
    order_total = 0
    order_weight = 0
    for products in cartdetails:
    ...
    conditionss = EngageboostShippingMastersSettings.objects.order_by("id").filter(website_id=website_id,
                                                                                   shipping_method_id='5',
                                                                                   isblocked='n', isdeleted='n').all()
    if conditionss:
        settings_info2 = ShippingMastersSettingsSerializer(conditionss, many=True)
        for settingsinfos in settings_info2.data:
            if country_id:
                conditions2 = EngageboostShippingTableRateOrderAmount.objects.order_by("id").filter(
                    shipping_masters_setting_id=settingsinfos["id"],
                    country_id__iregex=r"\y{0}\y".format(country_id)).all()
            else:
                conditions2 = EngageboostShippingTableRateOrderAmount.objects.order_by("id").filter(
                    shipping_masters_setting_id=settingsinfos["id"]).all()

            if state_id:
                conditions2 = conditions2.order_by("id").filter(shipping_masters_setting_id=settingsinfos["id"],
                                                                state_id__iregex=r"\y{0}\y".format(state_id)).all()
            else:
                conditions2 = conditions2

            if post_code:
                conditions2 = conditions2.order_by("id").filter(shipping_masters_setting_id=settingsinfos["id"],
                                                                zip_code__iregex=r"\y{0}\y".format(post_code)).all()
            else:
                conditions2 = conditions2
                ...
                for settings_orderamount_info in settings_order_amount_info.data:
                    if settings_orderamount_info["order_subtotal"]:
                        if float(order_total) >= float(settings_orderamount_info["order_subtotal"]):
                            data = {"shipping_type": "Table Rate Shipping",
                                    "shipping_setting_id": settings_orderamount_info["id"], "mthod_type": 1,
                                    "mthod_name": "orderwise",
                                    "flat_price": float(settings_orderamount_info["shipping_price"])}

                    if settings_orderamount_info["weight"]:
                        if float(order_weight) >= float(settings_orderamount_info["weight"]):
                            data = {"shipping_type": "Table Rate Shipping",
                                    "shipping_setting_id": settings_orderamount_info["id"], "mthod_type": 0,
                                    "mthod_name": "orderwise",
                                    "flat_price": float(settings_orderamount_info["shipping_price"])}
...
def rate_free(website_id='1', company_id=None, country_id=None, state_id=None):
    ...
    conditions = EngageboostShippingMastersSettings.objects.order_by("id").filter(website_id=website_id,
                                                                                  shipping_method_id='6', isblocked='n',
                                                                                  isdeleted='n').all()
    if country_id:
    ...
    if conditions:
        ...
        for settingsinfo in settings_info.data:
            if settingsinfo["minimum_order_amount"] is not None:
                data = {"shipping_setting_id": settingsinfo["id"],
                        "minimum_order_amount": float(settingsinfo["minimum_order_amount"])}
    return data
...
def generate_discount_conditions_coupon(website_id, user_group_id=None, coupon_code=None, warehouse_id=None):
    ...
    discount_multi_coupon = EngageboostDiscountMastersCoupons.objects.filter(isdeleted='n', coupon_code=coupon_code,
                                                                             is_used='n')
    if warehouse_id:
        discount_multi_coupon = discount_multi_coupon.filter(
            discount_master_id__warehouse_id__iregex=r"\y{0}\y".format(warehouse_id))
    ...
    if discount_multi_coupon:
        ...
        condition_discount_coupon_part = EngageboostDiscountMasters.objects.filter(coupon_code=coupon_code,
                                                                                   used_coupon=0)
        if warehouse_id:
            condition_discount_coupon_part = condition_discount_coupon_part.filter(
                warehouse_id__iregex=r"\y{0}\y".format(warehouse_id))
        condition_discount_coupon_part = condition_discount_coupon_part.all()

    if condition_discount_coupon_part:
        condition_discount_coupon_part = condition_discount_coupon_part.filter(website_id=website_id,
                                                                               disc_start_date__lte=now_utc,
                                                                               disc_end_date__gte=now_utc,
                                                                               isdeleted='n', isblocked='n',
                                                                               discount_master_type='1').all()
        if user_group_id:
            condition_discount_coupon_part = condition_discount_coupon_part.filter(
                customer_group__iregex=r"\y{0}\y".format(user_group_id)).all()

        if condition_discount_coupon_part:
            condition_discount_coupon_part_serialize = DiscountMasterSerializer(condition_discount_coupon_part,
                                                                                many=True)
            for condition_discount_coupon in condition_discount_coupon_part_serialize.data:
                discount_master_condition = EngageboostDiscountMastersConditions.objects.order_by("id").filter(
                    discount_master_id=condition_discount_coupon["id"]).all()
                if discount_master_condition:
                    discount_master_condition_serializer = DiscountConditionsSerializer(discount_master_condition,
                                                                                        many=True)
                ...
                discount_master_coupon = EngageboostDiscountMastersCoupons.objects.order_by("id").filter(
                    discount_master_id=condition_discount_coupon["id"], isdeleted='n', coupon_code=coupon_code).first()
            ...
            data = {"name": "invalid", "coupon_code": coupon_code,
                    "message": "Coupon code expired or Condition not satisfied", "status": 0}
...
def genrate_new_prodcut_with_discount_coupon(user_id=None, cartdetails=None, discount_array_coupon=None,
                                             order_total=None):
    for discount_array in discount_array_coupon:
        for product in cartdetails:
            ...
            if price_array[2] != "false":
                ...
                product["discount_price"] = float(product["discount_price"]) + float(price_array[1]) * float(
                    product["qty"])
                ...
                product["coupon_msg"] = price_array[3]
            else:
...
def check_in_prod_disc_coupon(user_id=None, ind_product=None, discount_array=None, cart_subtotal=None):
    ...
    msg = ""
    flag = "false"
    # print('======check_in_prod_disc_coupon======')
    # print('======check_in_prod_disc_coupon discount_array_condition======', discount_array_condition)
    for ind_cond in discount_array_condition:
        # print('======check_in_prod_disc_coupon ind_cond["value"]======', ind_cond["value"])
        # print('======check_in_prod_disc_coupon cart_subtotal======', cart_subtotal)
        if ind_cond["fields"] == -1:
            if cart_subtotal:
                if ind_cond["condition"] == "==":
                    flag = "true" if float(cart_subtotal) == float(ind_cond["value"]) else "false"
    ...
    # --------Binayak Start 07-04-2021-------#
    print("======user_id=======", user_id)
    # ---------------['Waiting Approval', 'Approved', 'Pending', 'Processing', 'Shipped', 'Completed', 'Partial Refund', 'Delivered']--------------#
    check_if_coupon_used = EngageboostOrdermaster.objects.filter(customer__auth_user_id=user_id,
                                                                 applied_coupon=discount_array['coupon_code'],
                                                                 order_status__in=[99, 20, 0, 100, 1, 4, 6, 13])
    # print("====check_if_coupon_used======", check_if_coupon_used.count())
    # print("====ind_product======", ind_product)
    # print("====discount_array======", discount_array['coupon_code'])
    # print("====usage_limit_per_user======", discount_array['usage_limit_per_user'])
    if discount_array['usage_limit_per_user'] != None and previos_flag == "true":
        if check_if_coupon_used.count() < int(discount_array['usage_limit_per_user']):
            previos_flag = "true"
        else:
            previos_flag = "false"
            msg = "Maximum times coupon usage limit reached"
    # --------Binayak End 07-04-2021-------#

    # print('previos_flag====', previos_flag)
    # print('ind_product["new_default_price_unit"]====', ind_product["new_default_price_unit"])
    # print('ind_product["discount_price_unit"]====', ind_product["discount_price_unit"])
    if previos_flag == "true":
        if discount_array["disc_type"] == 1:
            # ------Binayak Start 17-12-2020-------#
            discount_array["discountPrice"] = (float(ind_product["new_default_price_unit"]) + float(
                ind_product["tax_price_unit"])) * float(discount_array["amount"]) / float(100)
            # discount_array["discountPrice"]=float(ind_product["new_default_price_unit"])*float(discount_array["amount"])/float(100)
            # ------Binayak End 17-12-2020-------#
        else:
            discount_array["discountPrice"] = float(discount_array["amount"])
        ...
        if ind_product["new_default_price_unit"]:
            if float(ind_product["new_default_price_unit"]) <= float(0):
                if float(ind_product["new_default_price_unit"]) > float(discount_array["discountPrice"]):
                    default_price = float(ind_product["new_default_price_unit"]) - float(
                        discount_array["discountPrice"])
    ...
    return str(default_price) + "^" + str(discount_price) + "^" + str(previos_flag) + "^" + str(msg)


def genrate_new_prodcut_with_discount_coupon_order_total(user_id=None, cartdetails=None, discount_array_coupon=None,
                                                         order_total=None):
    set_of_response = []
    coupon_details_dict = {}
    for discount_array in discount_array_coupon:
        coupon_details = {"coupon_discount_amount": float(discount_array["amount"]),
                          "coupon_disc_type": discount_array["disc_type"], "coupon_name": discount_array["name"],
                          "coupon_code": discount_array["coupon_code"]}
        for product in cartdetails:
            return_price = check_in_prod_disc_coupon_order_total(user_id, product, discount_array, order_total)
            # print('return_price====>', return_price)
            price_array = return_price

            # ------Binayak Start 08-04-2021-----#
            # set_of_response.append(price_array)
            set_of_response.extend(price_array)
            # ------Binayak Start 08-04-2021-----#
...
def check_in_prod_disc_coupon_order_total_old(user_id=None, ind_product=None, discount_array=None, cart_subtotal=None):
    discount_array_condition = discount_array["DiscountMasterCondition"]
    product_array_disc_applied = []
    i = 0
    flag = "false"
...
def GetProductDiscount(self, request, format=None):
    ...
    for index in range(len(product_ids)):
        ...
        if str(company_state) != str(state_id):
            product_tax = {"tax_price_unit": float(tax_price_arr["tax_igst"]),
                           "tax_price": float(tax_price_arr["tax_igst"]) * float(product_detail_net["qty"]),
                           "tax_percentage": tax_price_arr["igst"], "tax_name": tax_price_arr["tax_name"],
                           "cgst": tax_price_arr["cgst"], "sgst": tax_price_arr["sgst"], "igst": tax_price_arr["igst"],
                           "cess": tax_price_arr["cess"], "is_igst": "yes", "tax_type": tax_price_arr["tax_type"]}
        else:
            product_tax = {"tax_price_unit": float(tax_price_arr["tax_igst"]),
                           "tax_price": float(tax_price_arr["tax_igst"]) * float(product_detail_net["qty"]),
                           "tax_percentage": tax_price_arr["igst"], "tax_name": tax_price_arr["tax_name"],
                           "cgst": tax_price_arr["cgst"], "sgst": tax_price_arr["sgst"], "igst": tax_price_arr["igst"],
                           "cess": tax_price_arr["cess"], "is_igst": "no", "tax_type": tax_price_arr["tax_type"]}

        product_detail_net = dict(product_detail_net, **product_tax)

        if product_detail_net["tax_type"] == "including":
            product_detail_net["new_default_price_unit"] = float(product_detail_net["new_default_price_unit"]) - float(
                product_detail_net["tax_price_unit"])
            product_detail_net["new_default_price"] = float(product_detail_net["new_default_price"]) - float(
                product_detail_net["tax_price"])
        # ********Tax Calculation (END)********#
    ...
    # ********Shipping Calculation********#
    shipping_amount_arr = {"shipping_amount": 0, "mthod_type": 0, "mthod_name": "orderwise", "handling_fees_type": 0,
                           "handling_price": 0}
    if cartdetails:
        shipping_flat = rate_flat(website_id, company_id, country_id, state_id, post_code, 'back')
        shipping_table = rate_table(website_id, company_id, cartdetails, country_id, state_id, post_code, 'back')
    ...
    if cartdetails and coupon_code is not None:
        ...
        if discount_array_coupon and discount_array_coupon[0]["name"] != "invalid":
            if discount_array_coupon[0]["disc_type"] != 3:
                if discount_array_coupon[0]["coupon_type"] == 2:
                    cartdetails = genrate_new_prodcut_with_discount_coupon(user_id, cartdetails, discount_array_coupon,
                                                                           order_total)
                else:
                    if discount_array_coupon[0]["used_coupon"] == 0:
                        cartdetails = genrate_new_prodcut_with_discount_coupon(user_id, cartdetails,
    ...
    if cartdetails:
        ...
        if coupon_code is not None:
            if discount_array_coupon and discount_array_coupon[0]["name"] != "invalid":
                ...
                applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                       "coupon_code": discount_array_coupon[0]["coupon_code"],
                                       "message": discount_array_coupon[0]["message"], "status": 0}
                applied_coupon.append(applied_coupon_dict)
        else:
            grandtotal = float(grandtotal)
            subtotal = float(grandtotal)
    ...
    orderamountdetails_dict = {"company_id": company_id, "website_id": website_id, "webshop_id": webshop_id,
                               ...
                               "shipping_charge": shipping_amount, "handling_charge": handling_charge,
                               "cod_charge": applied_cod_charge, "gross_discount": gross_discount_amount,
                               "min_amount": cod_min_amount, "max_amount": cod_max_amount, "grand_total": grandtotal,
                               "sub_total": subtotal, "net_total": nettotal, "cart_discount": cart_discount,
...
def payment_method_onepage_checkout_old(website_id):
    ...
    payment_setting_info_cond = EngageboostPaymentgatewaySettingInformation.objects.filter(website_id=website_id,
                                                                                           isblocked='n',
                                                                                           isdeleted='n').all()
    ...
    for payment_settinginfo in payment_setting_info.data:
        payment_setting_type = EngageboostPaymentgatewayTypes.objects.filter(
            id=payment_settinginfo["paymentgateway_type_id"]).first()
        payment_setting_method = EngageboostPaymentgatewayMethods.objects.filter(
            id=payment_settinginfo["paymentgateway_method_id"]).first()
...
def payment_method_onepage_checkout(website_id):
    ...
    website_methods = []
    payment_setting_info_list = []
    website_data_cond = EngageboostWebsitePaymentmethods.objects.filter(engageboost_company_website_id=website_id,
                                                                        engageboost_paymentgateway_method_id=16).last()
    ...
    payment_setting_info_cond = EngageboostPaymentgatewaySettingInformation.objects.filter(website_id=website_id,
                                                                                           ...
                                                                                           paymentgateway_method_id=16).all()
    payment_setting_info = PaymentgatewaySettingInformationSerializer(payment_setting_info_cond, many=True)
...
def getcurrencycode(webID):
    ...
    if webID > 0:
        ...
        if cur_code:
            ...
            rs_currency = EngageboostCurrencyRates.objects.filter(isbasecurrency='y',
                                                                  engageboost_company_website_id=webID).first()
...
def product_offer_discount_price(website_id, listing_products, user_id=None):
    ...
    if listing_products:
        ...
        for prod_details in listing_products:
            if prod_details['new_default_price'] and prod_details['new_default_price'] > 0 and prod_details[
                'disc_type']:
...
def GetCartDetails(company_id, website_id, customer_id=None, device_id=None, webshop_id=None, country_id=None,
                   state_id=None, post_code=None, user_id=None, user_group_id=None, coupon_code=None, warehouse_id=None,
                   str_from=None):
    # company_db = loginview.db_active_connection(request)
    ...
    global_setting_date = EngageboostGlobalSettings.objects.filter(website_id=1, isdeleted='n', isblocked='n').values(
        "applicable_tax", "timezone_id").first()
    applicable_tax_settings = global_setting_date["applicable_tax"]
    ...
    for cartdata in cart_data.data:
        ...
        if getproductforcart_net:
            ...
            # -----Binayak End 07-01-2021-----#

            cart_count = cart_count + int(cartdata['quantity'])
            try:
                if getproductforcart_net["new_default_price"]:
                    getproductforcart_net["new_default_price"] = float(getproductforcart_net["channel_price"]) * int(
                        cartdata['quantity'])
                    getproductforcart_net["new_default_price_unit"] = float(getproductforcart_net["channel_price"]) if \
                    getproductforcart_net["channel_price"] else 0.0
                ...
                getproductforcart_net["new_default_price"] = float(getproductforcart_net["default_price"]) * int(
                    cartdata['quantity'])
                getproductforcart_net["new_default_price_unit"] = float(getproductforcart_net["channel_price"]) if \
                getproductforcart_net["channel_price"] else 0.0
                getproductforcart_net["discount_price_unit"] = 0.00
        ...
        # print('discount_array_net======>', discount_array_net)

        # ********Tax Calculation********#

        if country_id != "" or state_id != "" or post_code != "":
            if applicable_tax_settings is not None and applicable_tax_settings.lower() == "gst":
                tax_price_arr = get_price_including_tax(website_id, company_id, product_detail_net, country_id,
                                                        state_id, post_code, 'back')
            if applicable_tax_settings is not None and applicable_tax_settings.lower() == "vat":
                tax_price_arr = get_price_including_tax_vat(website_id, company_id, product_detail_net, country_id,
                                                            state_id, post_code, 'back')

        company_info = EngageboostCompanyWebsites.objects.filter(id=website_id).values("state").first()
        if company_info:
            company_state = company_info["state"]
        else:
            company_state = 0
        if applicable_tax_settings is not None and applicable_tax_settings.lower() == "gst":
            if str(company_state) != str(state_id):
                ...
                product_tax = {"tax_price_unit": float(tax_price_arr["tax_igst"]),
                               ...
                               "tax_type": tax_price_arr["tax_type"]}
        if applicable_tax_settings is not None and applicable_tax_settings.lower() == "vat":
            str_tax_price = (float(product_detail_net["new_default_price_unit"]) * float(
                tax_price_arr["tax_rate"])) / 100
            str_tax_price = float(str_tax_price) * int(product_detail_net["qty"])
            tax_price_unit = 0
            if str(company_state) != str(state_id):
                str_tax_rate = float(tax_price_arr["tax_rate"])
                if tax_price_arr["tax_type"].lower() == "including tax":
                    tax_price_unit = float(product_detail_net["new_default_price_unit"]) - float(
                        product_detail_net["new_default_price_unit"]) * (100 / float(float(str_tax_rate) + 100))
                    str_tax_price = float(tax_price_unit) * int(product_detail_net["qty"])
                else:
                    tax_price_unit = (float(product_detail_net["new_default_price_unit"]) * float(
                        tax_price_arr["tax_rate"])) / 100
                    str_tax_price = float(tax_price_unit) * int(product_detail_net["qty"])

                product_tax = {"tax_price_unit": float(tax_price_unit), "tax_price": float(str_tax_price),
                               "tax_percentage": tax_price_arr["tax_rate"], "tax_name": tax_price_arr["tax_name"],
                               "tax_type": tax_price_arr["tax_type"]}
            else:
                str_tax_rate = float(tax_price_arr["tax_rate"])
                if tax_price_arr["tax_type"].lower() == "including tax":
                    tax_price_unit = 0
                    tax_price_unit = float(product_detail_net["new_default_price_unit"]) * (
                                100 / float(float(str_tax_rate) + 100))
                    str_tax_price = float(
                        float(product_detail_net["new_default_price_unit"]) - float(tax_price_unit)) * int(
                        product_detail_net["qty"])
                else:
                    str_tax_price = (float(product_detail_net["new_default_price_unit"]) * float(
                        tax_price_arr["tax_rate"])) / 100
                    str_tax_price = float(str_tax_price) * int(product_detail_net["qty"])
        ...
        if product_detail_net["tax_type"].lower() == "including tax":
            product_detail_net["new_default_price_unit"] = float(product_detail_net["new_default_price_unit"]) - float(
                tax_price_unit)
            product_detail_net["new_default_price"] = float(product_detail_net["new_default_price"]) - float(
                product_detail_net["tax_price"])
    ...
    shipping_amount_arr = {"shipping_amount": 0, "mthod_type": 0, "mthod_name": "orderwise", "handling_fees_type": 0,
                           "handling_price": 0}
    if cartdetails:
        shipping_flat = rate_flat(website_id, company_id, country_id, state_id, post_code, 'back', warehouse_id,
                                  str_from)
        shipping_table = rate_table(website_id, company_id, cartdetails, country_id, state_id, post_code, 'back')
        shipping_free = rate_free(website_id, company_id, country_id, state_id)
    ...
    if cartdetails:
        ...
        if coupon_code and coupon_code is not None:
                ...
                # -----Binayak End-----#
                    ...
                    if float(cart_discount) > 0:
                        ...
                        applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                               "coupon_code": discount_array_coupon[0]["coupon_code"],
                                               "disc_type": discount_array_coupon[0]["disc_type"],
                                               "amount": gross_discount_amount, "status": 1}
                    else:
                        if gross_discount_amount > 0 and float(discount_array_coupon[0]["amount"]) > 0:
                            print("++++++++++++k3")
                            # print("++++++++++++product_cart_discount==>", product_cart_discount)
                            # print("++++++++++++cart_discount==>", cart_discount)
                            ...
                            actual_discount = float(product_cart_discount) + float(cart_discount)
                            actual_discount = Decimal(actual_discount).quantize(Decimal('.00'))

                            if float(actual_discount) > 0:
                                applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                                       "coupon_code": discount_array_coupon[0]["coupon_code"],
                                                       "disc_type": discount_array_coupon[0]["disc_type"],
                                                       "amount": actual_discount, "status": 1}
                            else:
                                if 'coupon_msg' in coupon_details:
                                    if coupon_details['coupon_msg'] not in ('', None):
                                        applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                                               "coupon_code": discount_array_coupon[0]["coupon_code"],
                                                               "disc_type": discount_array_coupon[0]["disc_type"],
                                                               "message": coupon_details['coupon_msg'],
                                                               "status": 0}
                                        print("======in here 0======")
                                    else:
                                        applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                                               "coupon_code": discount_array_coupon[0]["coupon_code"],
                                                               "disc_type": discount_array_coupon[0]["disc_type"],
                                                               "message": "Minimum order amount not satisfied",
                                                               "status": 0}
                            # applied_coupon_dict={"name":discount_array_coupon[0]["name"],"coupon_code":discount_array_coupon[0]["coupon_code"],"disc_type":discount_array_coupon[0]["disc_type"], "amount":actual_discount,"status":1}
                        else:
                            print("++++++++++++k4")
                            print("++++++++cartdetails++++++++++", coupon_details)
                            if 'coupon_msg' in coupon_details:
                                if coupon_details['coupon_msg'] not in ('', None):
                                    applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                                           "coupon_code": discount_array_coupon[0]["coupon_code"],
                                ...
                                # if cartdetails
                                # print("=====cartdetails=======", cartdetails[0]['coupon_msg'])
                                if cartdetails[0]['coupon_msg'] not in ('', None):
                                    applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                                           "coupon_code": discount_array_coupon[0]["coupon_code"],
                                                           "disc_type": discount_array_coupon[0]["disc_type"],
                                                           "message": cartdetails[0]['coupon_msg'],
                                                           "status": 0}
                                else:
                                    applied_coupon_dict = {"name": discount_array_coupon[0]["name"],
                                                           "coupon_code": discount_array_coupon[0]["coupon_code"],
            ...
            # ------Binayak Start 17-12-2020------#
            cart_discount = float(grandtotal) - float(subtotal)
            # cart_discount = float(grandtotal)-float(subtotal) - float(total_tax)
            # ------Binayak End 17-12-2020------#
    ...
    # print('minimum_order_amount=====>', minimum_order_amount)
    # if float(subtotal)<float(minimum_order_amount):
    if float(subtotal) + float(cart_discount) - float(shipping_amount) < float(minimum_order_amount):
    ...
    orderamountdetails_dict = {
        ...
        "cart_discount": Decimal(cart_discount).quantize(Decimal('.00')) + Decimal(product_cart_discount).quantize(
            Decimal('.00')),
    ...
    data = {"cartdetails": cartdetails, "orderamountdetails": orderamountdetails, "applied_coupon": applied_coupon,
            "shipping_flat": shipping_flat, "shipping_table": shipping_table, "cart_count": cart_count}
    return Response(data)
    # return data
...
def get_discount_detalils(data_arr):
    ...
    try:
        if getproductforcart_net["new_default_price"]:
            getproductforcart_net["new_default_price"] = float(getproductforcart_net["new_default_price"]) * int(
                getproductforcart_net["qty"])
            getproductforcart_net["new_default_price_unit"] = float(getproductforcart_net["channel_price"]) if \
            getproductforcart_net["channel_price"] else 0.0
            getproductforcart_net["discount_price_unit"] = 0.00
            getproductforcart_net["discount_price"] = 0.00
