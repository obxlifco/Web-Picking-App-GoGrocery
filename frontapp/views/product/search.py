    ...
    def get(self, request, slug, format=None):
        ...
        if rs_product:
            ...
            products_discount_data = {
                "new_default_price": round(productsdiscount[0]['new_default_price'],2),
                "new_default_price_unit": round(productsdiscount[0]['new_default_price_unit'],2),
                "discount_price_unit": round(productsdiscount[0]['discount_price_unit'],2),
                "discount_price": round(productsdiscount[0]['discount_price'],2),
                "discount_amount": round(productsdiscount[0]['discount_amount'],2),
            ...
            if(cnt > 0):
                ...
                if str_type == "category":
                    ...
                    if category_ids is not None and category_ids!="":
                        ...
                        product_lst = EngageboostProductCategories.objects.filter(category_id__in=cat_ids, isblocked='n', isdeleted='n', product_id__isblocked='n', product_id__isdeleted='n').values_list('product_id',flat=True)
...
def ProductList(product_id, warehouse_id):
    ...
    if product_id:
        ...
        if product_ids is not None:
            ...
            for products in serializer.data:
                # Get Product Stock
                qs_stock = EngageboostProductStocks.objects.filter(product_id=products['id'], warehouse_id=warehouse_id).all().iterator()
                stock_data = ProductStocksSerializer(qs_stock, many=True)
                stock_data = stock_data.data
                products['product']['default_price'] = products['product']['channel_price']
                # Get variant Products
